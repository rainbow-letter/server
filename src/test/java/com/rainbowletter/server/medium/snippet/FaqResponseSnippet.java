...
public class FaqResponseSnippet {
	...
	public static final Snippet ADMIN_FAQ_RESPONSES = responseFields(
			...
			fieldWithPath("faqs[].sequence").type(JsonFieldType.NUMBER).description("정렬 순번"),
			fieldWithPath("faqs[].createdAt").type(JsonFieldType.STRING).description("생성일"),
			fieldWithPath("faqs[].updatedAt").type(JsonFieldType.STRING).description("수정일")
	);
